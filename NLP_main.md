...
<p style='color:#3d2002'>This paragraph aims to show some very important subtleties of text embedding machinery. Especially the differences between the old fashioned static (context independent) embedding known as <b>word2vec</b> and new one known as <b>transformer architecture</b>, that takes context into account.</p>

The human language is the highly context dependable stream of information. The semantic(meaning) of the words and phrases depends on the previous words and phrases.
Let's consider this simple textbook example - **"Apple shares rose on the news. Apple pie is delicious."** Any adult human easily catches the meaning of these two phrases. The first one is about financial market and company. The second one is about fruits and cooking. Speaking in the terms of semantic distances thy should be fart apart of each others, even though they contain similar words.
...
Now, let's see the result of distance measuring between two **Apples** with **transformer** model **"en_trf_bertbaseuncased_lg"** (405.8MB)
```python
...
As we can easily see the second(fruity) **Apple** makes a light stripe through all the first sentence. It means it has little in common with **Apple** company and economics. In addition we can see that two senteces forms the dark colour clusters that helps us to see they have different meanings.   
...
Despite the recent achievements in NLP, many unsolved problems exist today that require almost human level reasoning. Let's consider the simple task, that any educated human is able to resolve without problem.
...
As we can see the answers 1 and 2 almost don't have any difference apart of to words **charge** and **mass**. However this one word makes answer 1 completely wrong. At the same time the answer 3 is extremely good.

Model decides that answer 1 is similar to 2 and therefore is also true. It decides that way because it is unable to reason about physics.

How can we try to solve the problem, knowing that the models with physics knowledge doesn't exist?
...
For example, if the model before trying to match all possible options, tries to find the answer on the question and looks for the all available information. There is a chance that there won't be any data about electrical charges and gravitation.  

### Using additional context

This is the question answering test of the new transformer model. It's trained on the publicly available QuA dataset and isn't tuned for any particular task. Nevertheless, for some context-questions pairs it works amazingly good.
...
Here are my thoughts how to organise NLP service at the organisation scale.

At the moment, it is clear, that at the core of NLP service should be the big **transformer** model. It was shown, that  transformers NLP performance scales linearly with the size of training data. It goes almost withouht a sign of any plato. Thus, transformer models have a place to grow in the near future. 
Transformer models require substantial processing power to work. In production they run on the instances with GPU/TPU. The architecture, assumes that the main goal of transformer is to provide vector representation of the natural text messages.
